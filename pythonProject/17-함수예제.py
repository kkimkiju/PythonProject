# 순차검색
def search_list(a, x):  # 리스트와 값을 매개 변수로 전달 받음, 해당 값이 존재하면 인덱스 반환
...
# 기본값 인자 : 파이썬은 매 개변수에 대한 기본값을 정의 할 수 있음
# 함수 호출 시 인자값을 넣지 않으면 매개 변수의 기본값이 반영됨
...
#가변 매개 변수 : 함수의 매개 변수 앞에 *를 붙여주면 함수의 매개 변수를 몇개를 입력하든 상관없음
